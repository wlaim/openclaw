...
describe("chat view", () => {
  ...
  it("passes bare picker values through while the session resolves the unique catalog provider", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: false,
    ...
    expect(state.sessionsResult?.sessions[0]?.model).toBe("gemini-2.5-flash");
    expect(state.sessionsResult?.sessions[0]?.modelProvider).toBe("google");
    vi.unstubAllGlobals();
  });
